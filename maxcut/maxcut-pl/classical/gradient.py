...
class GradientOptimizer(object):
    ...
    def optimize(self, **kwargs):
        ...
        if params is None:
            raise ValueError("Expected 'params' passed as a keyword argument to be used by the cost function.")
        ...
        if steps is None:
            raise ValueError("Expected 'steps' to be passed a keyword argument for the number of steps the optimizer is to take towards convergence.")

        # Run the optmizer for the given number of steps
        for i in range(steps):
            params = self.optimizer.step(self.cost, params)

        val = self.cost(params)
        print(val)
